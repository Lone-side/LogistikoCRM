...
class ClientProfileAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        # Changelist N+1: portal_status hits obj.user, obligations_status hits
        # obj.obligation_settings (+ its M2M types via profiles), documents_count
        # counts documents. Pull them in one go.
        return (
            super().get_queryset(request)
            .select_related('user', 'obligation_settings')
            .prefetch_related(
    ...
    def documents_count(self, obj):
        ...
        # Uses the _documents_count annotation from get_queryset (no per-row query).
