...
> **Τρέξε το preflight ~30 δευτερόλεπτα ΜΕΤΑ το `up -d`, όχι αμέσως.** Ο
> Celery worker χρειάζεται λίγο για να δηλωθεί· αν το preflight προλάβει,
> βγάζει παροδικό `WARN celery: workers=0` που δεν σημαίνει τίποτα. Σε ένα
> go/no-go φύλλο αυτό μοιάζει με πρόβλημα ενώ δεν είναι — ξανατρέξε το. Χωρίς αυτό, «ενημέρωσα τον φάκελο αλλά ξέχασα το redeploy» είναι
...
Ενδοκοινοτικές) και 65 τύπους υποχρεώσεων: 23 πλήρως ρυθμισμένους (με
τις επίσημες ονομασίες — π.χ. «Περιοδική Φ.Π.Α. (Μήνα)», «ΑΠΔ (Κοινών)»)
και 42 του default catalog **χωρίς** προσυμπληρωμένη περιοδικότητα/
προθεσμία — αυτά τα ρυθμίζει ο λογιστής πριν την αυτόματη δημιουργία
υποχρεώσεων (μέχρι τότε είναι διαθέσιμα μόνο για χειροκίνητη επιλογή
ανά πελάτη). Σε υπάρχουσα εγκατάσταση, τα παλιά ονόματα (ΦΠΑ Μηνιαίο
κ.λπ.) μετονομάζονται αυτόματα στις επίσημες ονομασίες διατηρώντας
ρυθμίσεις και αναθέσεις.
